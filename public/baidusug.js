 ...
 * 请为 input 的父元素设置 position
 *
 * 支持的参数：
 * config.callback      选中结果回调函数: callback: (searchText, isPressEnterKey) => void;
 *                      因为回车键可能会触发表单的 submit(本组件默认做了 preventDefault 处理), 所以提供了 isPressEnterKey 参数
...
function BaiduSug(domId, config) {
  ...
  this.showResult = function (result, config) {
    ...
    var onClick = function (e) {
      self.onSubmit(e.target.innerText, false);
  ...
  this.onSubmit = function (text, isPressEnterKey) {
    var cb =
      this.config.callback ||
      function () {
        console.log('submit text: ' + text);
      };
    cb(text, isPressEnterKey);
    this.domInput.blur();
  };
    ...
    if (this.domInput) {
      ...
      self.domInput.addEventListener('keypress', function (e) {
        ...
        if (e.keyCode === 13) {
          self.currentLiIndex = -1;
          self.clearResult();
          self.onSubmit(self.getSearchText(), true);
      ...
      console.info('baidusug init success!');
    }
  };
